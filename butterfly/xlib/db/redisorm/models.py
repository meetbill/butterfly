...
from xlib.db.redisorm.query import ABSOLUTE
from xlib.db.redisorm.query import CONTINUOUS
from xlib.db.redisorm.query import Desc
from xlib.db.redisorm.query import Executor
from xlib.db.redisorm.query import FTS
from xlib.db.redisorm.query import Node
from xlib.db.redisorm.search import Tokenizer
from xlib.db.redisorm.utils import basestring_type
from xlib.db.redisorm.utils import decode
from xlib.db.redisorm.utils import decode_dict_keys
from xlib.db.redisorm.utils import encode
from xlib.db.redisorm.utils import PY3
from xlib.db.redisorm.utils import unicode_type
...
class AutoIncrementField(IntegerField):
    ...
    def _generate_key(self):
        ...
        return self.model_class._database_.incr(key)
...
class Query(object):
    ...
    def _base_key(self):
        ...
        if self.model_class._namespace_:
            return '%s|%s:' % (self.model_class._namespace_, model_name)
        return '%s:' % model_name
...
class BaseIndex(object):
    ...
    def __init__(self, field):
        self.field = field
        self._database_ = self.field.model_class._database_
...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if 'database' in attrs:
            warn('"database" has been deprecated in favor of "_database_" '
                 'for Walrus models.', DeprecationWarning)
            attrs['_database_'] = attrs.pop('database')
        if 'namespace' in attrs:
            warn('"namespace" has been deprecated in favor of "_namespace_" '
                 'for Walrus models.', DeprecationWarning)
            attrs['_namespace_'] = attrs.pop('namespace')
...
class Model(_with_metaclass(BaseModel)):
    ...
    * Full-text search
    ...
    def to_hash(self):
        ...
        raw model data.
        """
        return self._database_.Hash(self.get_hash_id())
    ...
    def query(cls, expression=None, order_by=None):
        ...
        if expression is not None:
            executor = Executor(cls._database_)
    ...
    def load(cls, primary_key, convert_key=True):
        ...
        if not cls._database_.hash_exists(primary_key):
            raise KeyError('Object not found.')
        raw_data = cls._database_.hgetall(primary_key)
        if PY3:
    ...
    def delete(self, for_update=False):
        ...
        self._database_.delete(hash_key)

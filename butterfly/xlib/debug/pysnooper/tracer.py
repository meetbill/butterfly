...
class Tracer(object):
    ...
    def trace(self, frame, event, arg):
        ...
        if self.last_source_path != source_path:
            # 31 red
            line = colored(u'{indent}Source path:... {source_path}'.format(**locals()), color=31)
            self.write(line)
        ...
        for name, value_repr in local_reprs.items():
            if name not in old_local_reprs:
                # 青色
                line = colored('{indent}{newish_string}{name} = {value_repr}'.format(**locals()), color=36)
                self.write(line)
        ...
        if ended_by_exception:
            ...
            event 为 call 和 line , return 时都会输出为绿色
            event 为 exception 是，会输出 @@@@@@@
            """
            # green
            if event == "call":
                line = u'{indent}>>>>>|{timestamp} {thread_info}{event:9} {line_no:4} {source_line}'.format(**locals())
            elif event == "line":
                line = u'{indent}------{timestamp} {thread_info}{event:9} {line_no:4} {source_line}'.format(**locals())
            elif event == "return":
                line = u'{indent}|<<<<<{timestamp} {thread_info}{event:9} {line_no:4} {source_line}'.format(**locals())
